...
function Events() {
  ...
  return (
    <div className="pt-[20vh] p-10 min-h-screen bg-gray-50">
      <h1 className="text-4xl font-bold mb-10 text-center">All Events</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {sortedEvents.map((event, idx) => (
          <div key={idx} className="bg-white rounded-xl shadow-md overflow-hidden flex flex-col md:flex-row">
            <img 
              src={event.image} 
              alt={event.title} 
              className="w-full md:w-1/3 h-64 object-contain bg-white cursor-pointer" 
              onClick={() => { setModalImg(event.image); setModalOpen(true); }}
            />
            <div className="p-6 flex-1 flex flex-col justify-between">
              ...
              <button className="mt-2 px-4 py-2 bg-orange-600 text-white rounded hover:bg-orange-700 transition">{event.type === 'Upcoming' ? 'Register' : 'View Photos'}</button>
            </div>
          </div>
        ))}
      </div>
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50" onClick={() => setModalOpen(false)}>
          <img src={modalImg} alt="Event" className="max-w-3xl max-h-[80vh] rounded-lg shadow-lg" />
        </div>
      )}
    </div>
  );
